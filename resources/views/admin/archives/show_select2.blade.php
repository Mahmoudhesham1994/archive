    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.messages.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('msg_date') ? 'is-invalid' : '' }}" type="text" name="msg_date" id="msg_date" value="{{ old('msg_date') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('rel_num') ? 'is-invalid' : '' }}" name="rel_num_id" id="rel_num_id">
                    @foreach($rel_nums as $id => $rel_num)
                        <option value="{{ $id }}" {{ old('rel_num_id') == $id ? 'selected' : '' }}>{{ $id }}{{ $rel_num }}</option>
                    @endforeach
                </select>
