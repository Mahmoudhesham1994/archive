    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.messages.update", [$message->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input readonly class="form-control {{ $errors->has('num') ? 'is-invalid' : '' }}" type="text" name="num" id="num" value="{{ old('num', $message->num) }}" required>
                @if($errors->has('num'))
              ...
              <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('msg_date') ? 'is-invalid' : '' }}" type="text" name="msg_date" id="msg_date" value="{{ old('msg_date', $message->msg_date) }}" required>
...
-->
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('msg_type') ? 'is-invalid' : '' }}" name="msg_type_id" id="msg_type_id" required>
                    @foreach($msg_types as $id => $msg_type)
                        <option value="{{ $id }}" {{ ($message->msg_type ? $message->msg_type->id : old('msg_type_id')) == $id ? 'selected' : '' }}>{{ $msg_type }}</option>
                    @endforeach
                </select> 
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('doc_type') ? 'is-invalid' : '' }}" name="doc_type_id" id="doc_type_id">
                    @foreach($doc_types as $id => $doc_type)
                        <option value="{{ $id }}" {{ ($message->doc_type ? $message->doc_type->id : old('doc_type_id')) == $id ? 'selected' : '' }}>{{ $doc_type }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('msg_title') ? 'is-invalid' : '' }}" type="text" name="msg_title" id="msg_title" value="{{ old('msg_title', $message->msg_title) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('msg_subject') ? 'is-invalid' : '' }}" name="msg_subject" id="msg_subject">{{ old('msg_subject', $message->msg_subject) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('msg_desc') ? 'is-invalid' : '' }}" type="text" name="msg_desc" id="msg_desc" value="{{ old('msg_desc', $message->msg_desc) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('from_contact') ? 'is-invalid' : '' }}" name="from_contact_id" id="from_contact_id">
                    @foreach($from_contacts as $id => $from_contact)
                        <option value="{{ $id }}" {{ ($message->from_contact ? $message->from_contact->id : old('from_contact_id')) == $id ? 'selected' : '' }}>{{ $from_contact }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('priority') ? 'is-invalid' : '' }}" name="priority_id" id="priority_id">
                    @foreach($priorities as $id => $priority)
                        <option value="{{ $id }}" {{ ($message->priority ? $message->priority->id : old('priority_id')) == $id ? 'selected' : '' }}>{{ $priority }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('need_replay') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="need_replay" id="need_replay" value="1" {{ $message->need_replay || old('need_replay', 0) === 1 ? 'checked' : '' }}>
                </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('replay_date') ? 'is-invalid' : '' }}" type="text" name="replay_date" id="replay_date" value="{{ old('replay_date', $message->replay_date) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('rel_num') ? 'is-invalid' : '' }}" name="rel_num_id" id="rel_num_id">
                    @foreach($rel_nums as $id => $rel_num)
                        <option value="{{ $id }}" {{ ($message->rel_num ? $message->rel_num->id : old('rel_num_id')) == $id ? 'selected' : '' }}>{{ $rel_num }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sent_tos') ? 'is-invalid' : '' }}" name="sent_tos[]" id="sent_tos" multiple>
                    @foreach($sent_tos as $id => $sent_to)
                        <option value="{{ $id }}" {{ (in_array($id, old('sent_tos', [])) || $message->sent_tos->contains($id)) ? 'selected' : '' }}>{{ $sent_to }}</option>
                    @endforeach
                </select>
          ...
          {{--   <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('forward_to') ? 'is-invalid' : '' }}" name="forward_to_id" id="forward_to_id">
                    @foreach($forward_tos as $id => $forward_to)
                        <option value="{{ $id }}" {{ ($message->forward_to ? $message->forward_to->id : old('forward_to_id')) == $id ? 'selected' : '' }}>{{ $forward_to }}</option>
                    @endforeach
                </select>
...
<!--                 <a href="/admin/messages/delete/{{$media->id}}">{{$media->id}}حذف</a>-->
                <a class="btn btn-danger" onclick="return confirm('هل متأكد من الحذف?')" href="/INOUT/admin/messages/delete/{{$media->id}}"><i class="fa fa-trash"></i></a>
